...
ExprResult Sema::ActOnMoovExpr(SourceLocation MoovLoc, Expr *Operand) {
  assert(Operand && "moov operand cannot be null");
  
  // 1. Resolve any placeholder expressions (e.g. overloaded functions, etc.)
  ExprResult SubExpr = Operand;
  if (SubExpr.get()->getType()->isPlaceholderType()) {
    SubExpr = CheckPlaceholderExpr(SubExpr.get());
    if (SubExpr.isInvalid())
